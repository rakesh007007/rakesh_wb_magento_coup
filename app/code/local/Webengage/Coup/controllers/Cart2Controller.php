 ...
 */
class  Webengage_Coup_Cart2Controller extends Mage_Core_Controller_Front_Action
{
    /**
    ...
    public function couponRakeshAction()
    {
        /**
        ...
        try {
           ...
           Mage::getModel('checkout/session')->getQuote()->getShippingAddress()->setCollectShippingRates(true);
            Mage::getModel('checkout/session')->getQuote()->setCouponCode($isCodeLengthValid ? $couponCode : '')
                ->collectTotals()
                ->save();
            ...
            if ($codeLength) {
                if ($isCodeLengthValid && $couponCode == Mage::getModel('checkout/session')->getQuote()->getCouponCode()) {
                    //////Mage::log('valid h ji');
                       //////Mage::log(Mage::getModel('checkout/session')->getQuote()->getCouponCode());
                       //////Mage::log($couponCode);
                    $arrcp=$arrcp.''.Mage::getModel('checkout/session')->getQuote()->getCouponCode();
                     Mage::getModel('checkout/session')->getQuote()->setCouponCode('')
                     ->collectTotals()
                      ->save();
                    $ans = array();
                    ...
                    //////Mage::log('invalid h ji');
                }
            } else {
                //////Mage::log('iinvalid h ji');
            }
    ...
    public function ajaxUrlAction() {
        ...
        $ty = $helper->MytestForAjax($itemId);
        echo($ty);
     ...
     * @throws Exception
     */
    public function addcouponinternalAction($coup)
    {
        /**
         * No reason continue with empty shopping cart
         */
    ...
    public function addAction()
    {
        ...
        try {
            ...
            $cart->save();
            if (isset($params['coup'])) {
                //logic for coupon add here 
                $this->addcouponinternalAction($params['coup']);
            }
    ...
    public function deleteAction()
    {
        ...
        if ($id) {
            try {
				...
				$cartItems = $quote->getQuote()->getAllVisibleItems();
				foreach($cartItems as $item){
					$itemId = $item->getItemId();
					if($itemId==$id){
						//Mage::log('sucker');
						$boolr =Mage::helper('webengage_coup')->hola($item);
						 $price1=($boolr['price']);
				  $boolrJson=json_encode($boolr);
				  $sku1=($boolr['sku']);
				  $name1=($boolr['name']);
				  $categ1 = $boolr['categ'];
				  $cartcount = $boolr['cartcount'];
				  $cartqty = $boolr['cartqty'];
				  $cartsubtotal = $boolr['cartsubtotal'];
         ...
         $this->_redirectUrl((Mage::getUrl('*/*')).'?rakesh=true'.'&'.'price='.$price1.'&'.'sku='.$sku1.'&'.'name='.$name1.'&'.'categ='.$categ1.'&'.'cartcount='.$cartcount.'&'.'cartqty='.$cartqty.'&'.'cartsubtotal='.$cartsubtotal.'&'.'licenseCode='.$licenseCode);
    }
